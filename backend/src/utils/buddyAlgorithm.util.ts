...
export const buddyAlgorithm = (long: number, lat: number, level: number, age: number, users: IUser[]) => {
    // Filter users who have completed their profile for buddy matching
    const eligibleUsers = users.filter(user => isUserReadyForBuddyMatching(user));
